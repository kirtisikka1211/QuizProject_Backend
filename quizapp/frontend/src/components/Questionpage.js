...
const Questionpage = () => {
  ...
  const handleContinue = () => {
    const pageno=JSON.stringify(currentQuestionIndex+1)
    axios.post('http://localhost/api/unprompted/',{"user":roll_no,"action":"Continue","page":pageno,"time":curtime})
    .then(response => {
      console.log(response.data); 
    })
    .catch(error => {
  ...
  const handleSubmit = () => {
    const pageno=JSON.stringify(currentQuestionIndex+1)
    axios.post('http://localhost/api/unprompted/',{"user":roll_no,"action":"End","page":pageno,"time":curtime})
    .then(response => {
      console.log(response.data); 
    })
    .catch(error => {
