...
const Questionpage = () => {
  ...
  const handleOptionClick = (option,index) => {
    ...
    // console.log(details);
    axios.post('http://localhost/api/noassistance/',details)
    .then(response => {
  ...
  const handleContinue = () => {
    const pageno=JSON.stringify(currentQuestionIndex+1)
    axios.post('http://localhost/api/noassistance/',{"user":roll_no,"action":"Continue","page":pageno,"time":curtime})
    .then(response => {
      console.log(response.data); 
    })
    .catch(error => {
  ...
  const handleSubmit = () => {
    const pageno=JSON.stringify(currentQuestionIndex+1)
    axios.post('http://localhost/api/noassistance/',{"user":roll_no,"action":"End","page":pageno,"time":curtime})
    .then(response => {
      console.log(response.data); 
    })
    .catch(error => {
